...
from ._base import (page, unit_for, counts_for, _js,
                    PAPER, INK, MUTED, FAINT, GRID, HERO, DATA, DATA2,
                    FAINTDATA, LAB, FLOOR, TRACK, BEAD)
...
_F1_JS = """\
(()=>{
const L={{LABELS}},V={{VALUES}},C={{COUNTS}},STEP={{STEP}},MAXIDX={{MAXIDX}};
obsReveal('ch',s=>{
  const base=266,HW=14,step=STEP;
  const x0=i=>{const gap=Math.min(56,340/Math.max(1,L.length));return 30+i*gap};
  L.forEach((name,i)=>{
    const x=x0(i),n=C[i],ink=i===MAXIDX?HERO:DATA;
    for(let k=0;k<n;k++){
      const y=base-k*step,w=HW-1.5+rnd(k+1,i+2)*3;
      el(s,'line',{x1:x-w,y1:y,x2:x+w,y2:y,stroke:ink,'stroke-width':1.8,
        opacity:.85+rnd(k+2,i+4)*.15,class:'fade',style:`animation-delay:${i*.08+k*.012}s`});
      if(k%5===4)el(s,'circle',{cx:x+HW+4.5,cy:y,r:.8,fill:FAINT,
  ...
  txt(s,{x:200,y:306,'font-size':7,'font-weight':600,fill:FAINT,'text-anchor':'middle',
...
obsReveal('ch',s=>{
  ...
  vs.forEach((v,d)=>{
    ...
    const dot=el(s,'circle',{cx:x(d),cy:map(v),r:big?5.2:3.1,
      fill:weekend?PAPER:(big?HERO:DATA),stroke:big?HERO:DATA,'stroke-width':weekend?1.4:0,
...
obsReveal('ch',s=>{
  ...
  vs.forEach((v,d)=>{
    el(s,'line',{x1:x(d),y1:base,x2:x(d),y2:map(v),
      stroke:d===peak?HERO:DATA,'stroke-width':d===peak?1.6:1,
      opacity:d===peak?1:.8+rnd(d+1,7)*.2,
      class:'fade',style:`animation-delay:${d*.014}s`});
  });
  const pts=vs.map((v,d)=>`${x(d)} ${map(v)}`).join(' L ');
  el(s,'path',{d:'M'+pts,fill:'none',stroke:DATA,'stroke-width':2,pathLength:1,
    class:'draw',style:'animation-delay:.4s;animation-duration:1.2s'});
  const pd=el(s,'circle',{cx:x(peak),cy:map(vs[peak]),r:4.2,fill:HERO,
...
obsReveal('ch',s=>{
  ...
  P.forEach((v,si)=>{
    for(let k=0;k<v;k++){
      ...
      el(s,'line',{x1,y1,x2,y2,stroke:SH[si%SH.length],'stroke-width':1.9,
        class:'fade',style:`animation-delay:${idx*.012}s`});
      if(idx%10===0){
        const [dx,dy]=pol(cx,cy,R0-5,a);
        el(s,'circle',{cx:dx,cy:dy,r:.8,fill:FAINT,class:'fade',style:`animation-delay:${idx*.012}s`});
      }
    }
  ...
  txt(s,{x:200,y:296,'font-size':7,'font-weight':600,fill:FAINT,'text-anchor':'middle',
    'letter-spacing':'.12em',class:'fade',style:'animation-delay:1.1s'},'{{NOTE}}');
});
})();
"""
_GRAY = ["#22211F", "#8F8E86", "#C0BFB7"]
...
def render_F4(labels, values, title, sub, src, y_label="单位", note=None):
    ...
    shades = [HERO if i == max_i else _GRAY[i % len(_GRAY)] for i in range(len(pcts))]
    if note is None:
        note = "一格 = 1% · 12 点钟为零 · 顺时针读 · 每第 10 格一个圆点"
    js = (_F4_JS
          .replace("{{LABELS}}", _js(labels)).replace("{{PCTS}}", _js(pcts))
          .replace("{{SHADES}}", _js(shades)).replace("{{TOTAL}}", str(total_disp))
          .replace("{{CENTER}}", "一格 = 1%")
          .replace("{{NOTE}}", note))
    return page(title, sub, src, js)
...
_F5_JS = """\
(()=>{
const L={{LABELS}},V={{VALUES}},C={{COUNTS}},GAP={{GAP}},PX={{PX}},MAXC={{MAXC}},MAXIDX={{MAXIDX}};
obsReveal('ch',s=>{
  const y0=i=>54+i*GAP,X0=100;
  L.forEach((name,i)=>{
    const y=y0(i),n=C[i],rowInk=i===MAXIDX?HERO:DATA;
    txt(s,{x:X0-6,y:y+3,'font-size':8,'font-weight':700,fill:LAB,'text-anchor':'end',
      'letter-spacing':'.08em',class:'fade',style:`animation-delay:${i*.08}s`},name);
    ...
    for(let k=0;k<n;k++){
      ...
      el(s,'line',{x1:x,y1:y+9,x2:x,y2:y+9-h,stroke:rowInk,'stroke-width':1.7,
        opacity:.85+rnd(k+3,i+5)*.15,class:'fade',style:`animation-delay:${i*.08+k*.012}s`});
      if(k%5===4)el(s,'circle',{cx:x,cy:y+13,r:.8,fill:FAINT,
        class:'fade',style:`animation-delay:${i*.08+k*.012}s`});
    }
  ...
  txt(s,{x:200,y:308,'font-size':7,'font-weight':600,fill:FAINT,'text-anchor':'middle',
    'letter-spacing':'.12em',class:'fade',style:'animation-delay:.9s'},'{{NOTE}}');
});
})();
"""
...
obsReveal('ch',s=>{
  ...
  L.forEach((name,i)=>{
    ...
    for(let k=0;k<CA[i];k++){
      ...
      el(s,'line',{x1:xa-w,y1:y,x2:xa+w,y2:y,stroke:FAINTDATA,'stroke-width':1.8,
        opacity:.75+rnd(k+2,i+3)*.25,class:'fade',style:`animation-delay:${i*.08+k*.01}s`});
    }
    for(let k=0;k<CB[i];k++){
      const y=base-k*step,w=HW-1.2+rnd(k+1,i+7)*2.4;
      el(s,'line',{x1:xb-w,y1:y,x2:xb+w,y2:y,stroke:DATA,'stroke-width':1.8,
        opacity:.85+rnd(k+2,i+8)*.15,class:'fade',style:`animation-delay:${.15+i*.08+k*.01}s`});
    }
    const topB=base-(CB[i]-1)*step;
    const num=txt(s,{x:xb,y:topB-9,'font-size':10.5,'font-weight':800,fill:HERO,'text-anchor':'middle',
      class:'fade',style:`animation-delay:${.5+i*.08}s`},now);
    tip(num,`${name} — ${was} → ${now} {{YLABEL}}`);
    txt(s,{x:xa,y:base-(CA[i]-1)*step-9,'font-size':8.5,'font-weight':700,fill:FAINTDATA,'text-anchor':'middle',
...
const SHADE={{SHADES}},SHADEL={{SHADEL}};
obsReveal('ch',s=>{
  ...
  L.forEach((name,i)=>{
    ...
    V[i].forEach((v,si)=>{
      ...
      for(let k=0;k<n;k++){
        ...
        el(s,'line',{x1:x-w,y1:y,x2:x+w,y2:y,stroke:SHADE[si],'stroke-width':1.8,
          opacity:.85+rnd(k+2,i+si+4)*.15,class:'fade',
...
def render_F7(labels, seg_names, values, title, sub, src, y_label="单位", note=None):
    ...
    shades = ["#F5572F", "#22211F", "#8F8E86"]
    shade_l = ["#F5572F", "#22211F", "#6E6D66"]
    if note is None:
    ...
    js = (_F7_JS
          ...
          .replace("{{SHADES}}", _js(shades)).replace("{{SHADEL}}", _js(shade_l))
...
_F9_JS = """\
(()=>{
const L={{LABELS}},V={{VALUES}},IS={{IS}},STEP={{STEP}},NETIDX={{NETIDX}};
obsReveal('ch',s=>{
  ...
  rows.forEach(([name,v,lo,hi],i)=>{
    const x=x0(i),isTotal=IS[i],neg=v<0&&!isTotal,ink=i===NETIDX?HERO:DATA;
    const from=isTotal?0:lo,to=isTotal?v:hi,n=Math.abs(to-from);
    for(let k=0;k<n;k++){
      const y=yOf(from+k),w=HW-1.2+rnd(k+1,i+2)*2.4;
      if(neg)el(s,'line',{x1:x-w,y1:y,x2:x+w,y2:y,stroke:DATA2,'stroke-width':1.8,
        'stroke-dasharray':'2.5 2.5',opacity:.85,class:'fade',style:`animation-delay:${i*.12+k*.014}s`});
      else el(s,'line',{x1:x-w,y1:y,x2:x+w,y2:y,stroke:ink,'stroke-width':1.8,
        opacity:.85+rnd(k+2,i+4)*.15,class:'fade',style:`animation-delay:${i*.12+k*.014}s`});
    }
    if(i<rows.length-1){
      const nx=x0(i+1),lvl=isTotal?v:(neg?lo:hi);
      el(s,'line',{x1:x+HW+2,y1:yOf(lvl),x2:nx-HW-2,y2:yOf(lvl),stroke:FAINT,
    ...
    const num=txt(s,{x,y:topY-8,'font-size':10,'font-weight':800,
      fill:neg?DATA2:INK,'text-anchor':'middle',class:'fade',style:`animation-delay:${.4+i*.12}s`},
      (neg?'−':'')+Math.abs(v));
  ...
  el(s,'line',{x1:30,y1:base+4,x2:370,y2:base+4,stroke:GRID,'stroke-width':.8,class:'fade'});
  txt(s,{x:200,y:302,'font-size':7,'font-weight':600,fill:FAINT,'text-anchor':'middle',
...
obsReveal('ch',s=>{
  ...
  for(let k=0;k<100;k++){
    ...
    const len=inked?15+rnd(k+1,3)*6:5+rnd(k+1,7)*2.5;
    const [x1,y1]=pol(cx,cy,R0,a),[x2,y2]=pol(cx,cy,R0+len,a);
    el(s,'line',{x1,y1,x2,y2,stroke:inked?DATA:FLOOR,'stroke-width':inked?1.9:.6,
      class:'fade',style:`animation-delay:${k*.012}s`});
  }
  [25,50,75,100].forEach(m=>{
    const a=A0+m/100*SW,[dx,dy]=pol(cx,cy,R0-7,a),[tx2,ty2]=pol(cx,cy,R0-19,a);
    el(s,'circle',{cx:dx,cy:dy,r:1,fill:FAINT,class:'fade',style:'animation-delay:.8s'});
    txt(s,{x:tx2,y:ty2+3,'font-size':7,'font-weight':600,fill:FAINT,'text-anchor':'middle',
      class:'fade',style:'animation-delay:.85s'},m);
  });
  const aT=A0+GOAL/100*SW,[ex,ey]=pol(cx,cy,R0+20,aT);
  el(s,'circle',{cx:ex,cy:ey,r:3.2,fill:HERO,class:'pop',style:'animation-delay:1.1s'});
  ...
  txt(s,{x:200,y:300,'font-size':7,'font-weight':600,fill:FAINT,'text-anchor':'middle',
    'letter-spacing':'.12em',class:'fade',style:'animation-delay:1.2s'},'{{NOTE}}');
});
})();
"""
...
obsReveal('ch',s=>{
  ...
  L.forEach((name,i)=>{
    ...
    el(s,'line',{x1:X0-6,y1:y,x2:X1+6,y2:y,stroke:TRACK,'stroke-width':.7,
      class:'fade',style:`animation-delay:${i*.08}s`});
    const n=Math.max(0,Math.round(Math.abs(was-now)/Math.max(1,UNIT)));
    for(let k=0;k<n;k++){
      const t=(k+.5)/n,x=xb+t*(xa-xb),yy=y+(rnd(k+1,i+3)-.5)*2.6;
      el(s,'circle',{cx:x,cy:yy,r:1.5+rnd(k+2,i+4)*.9,fill:BEAD,opacity:.85,
        class:'pop',style:`animation-delay:${.3+i*.08+k*.03}s`});
    }
    el(s,'circle',{cx:xa,cy:y,r:4.2,fill:PAPER,stroke:DATA,'stroke-width':1.3,
      class:'pop',style:`animation-delay:${.2+i*.08}s`});
    const after=el(s,'circle',{cx:xb,cy:y,r:4.6,fill:HERO,
      class:'pop',style:`animation-delay:${.6+i*.08}s`});
    tip(after,`${name} — ${was} → ${now} {{YLABEL}}`);
    txt(s,{x:xa+10,y:y-8,'font-size':8.5,'font-weight':700,fill:FAINTDATA,
  ...
  txt(s,{x:X0,y:290,'font-size':7,'font-weight':600,fill:FAINT,class:'fade'},'{{LEFT}}');
  txt(s,{x:X1,y:290,'font-size':7,'font-weight':600,fill:FAINT,'text-anchor':'end',
    class:'fade'},'{{RIGHT}}');
  txt(s,{x:200,y:308,'font-size':7,'font-weight':600,fill:FAINT,'text-anchor':'middle',
    'letter-spacing':'.12em',class:'fade',style:'animation-delay:1s'},'{{NOTE}}');
});
